...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      show () {
        this.showFlag = true
        // 刚开始页面是display为none的，scroll的高度计算不对，显示的时候要手动refresh才能滚动
        setTimeout(() => {
      ...
      selectSong (song, index) {
        if (index !== 0) {
          // 传入的song是一个具有song实例属性的对象，不是一个真正的song实例
          const songInstance = new Song(song.id, song.mid, song.singer, song.name, song.album, song.duration, song.image, song.url)
          this.insertSong(songInstance)
          this.showTip()
        }
      },
      showTip () {
